...
const SurveyCard = ({survey}) => {
  ...
  return (
    <div
  key={survey.id}
  // Make the entire card a keyboard-focusable button for accessibility
  role="button"
  tabIndex="0"
  onClick={() => {
  ...
  className="
    ...
    cursor-pointer
    focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 // Focus styles
  "
>
  ...
  {survey.image_url ? (
    <img
      ...
      alt={survey.name}
      className="h-48 w-full object-cover" // Fixed height and object-cover to prevent distortion
    />
  ) : (
    // Placeholder for surveys without an image
